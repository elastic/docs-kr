---
id: esG7-03-03-master-data-nodes
slug: /krEsguide7/esG7-03-03-master-data-nodes
title: "3.3 마스터 노드와 데이터 노드 - Master & Data Nodes"
...
date: 2022-01-25
tags: ['system', 'architecture', 'index', 'shards']
---
...
<DocTabs>
  <DocTab name="Node-1 (마스터)">
```yaml
node.master: true
node.data: false
```
  </DocTab>
  <DocTab name="Node-2 (데이터)">
```yaml
node.master: false
node.data: true
```
  </DocTab>
  <DocTab name="Node-3 (데이터)">
```yaml
node.master: false
node.data: true
```
  </DocTab>
  <DocTab name="Node-4 (데이터)">
```yaml
node.master: false
node.data: true
```
...
![마스터 전용 노드와 데이터 노드 구분](../assets/03-03_master_data_nodes_1.png)

Kibana의 Monitoring 화면에서 노드의 역할들을 확인할 수 있습니다. 대부분의 Elastic Stack 모니터링 도구에서 마스터 노드는 별(⭑) 표시로 구분합니다.

![Kibana 모니터링 화면에서 마스터 노드와 데이터 노드 확인](../assets/03-03_master_data_nodes_2.png)
...
![네트워크 단절로 인한 클러스터 분리](../assets/03-03_master_data_nodes_3.png)
...
![클러스터 분리 시 마스터 노드가 절반 이상인 클러스터만 생존](../assets/03-03_master_data_nodes_4.png)
